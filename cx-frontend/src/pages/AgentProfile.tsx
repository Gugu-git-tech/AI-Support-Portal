// Agent profile settings (only profile settings per spec)
...
export default function AgentProfile() {
  ...
  return (
    <DashboardShell title="Profile Settings" navItems={nav} requiredRole="agent">
      ...
      <div className="grid lg:grid-cols-3 gap-6 mt-6">
        <div className="glass rounded-2xl p-6 neon-border text-center">
          <div className="mx-auto h-24 w-24 rounded-full bg-gradient-neon flex items-center justify-center text-4xl glow-blue">
            {user.avatar ?? "🤖"}
          </div>
              ...
              {avatars.map((a) => (
                <button
                  ...
                  className={cn(
                    ...
                    user.avatar === a.emoji ? "border-primary bg-gradient-neon glow-purple" : "border-border bg-secondary/40",
                  )}
                  title={a.label}
                >
                  {a.emoji}
